...
use super::capsuled_parser::{angle_diff_q8, check_sync, generate_flag, generate_quality};
...
fn calc_angle_offset_q16(dist: u32) -> i32 {
    if dist >= 50 * 4 {
        ...
        return (deg_to_rad_q16(8f64) - ((k2 << 6) as f64) - (((k2 * k2 * k2) / 98304) as f64))
            as i32;
    } else {
...
fn varbit_scale_decode(scaled: u32) -> (u32, u32) {
    for i in 0..RPLIDAR_VARBIT_SCALE_SCALE_BASES.len() {
        ...
        if remain >= 0 {
            return (target_base + ((remain << scale_level) as u32), scale_level);
        }
    }
